...
const instance = axios.create({    //创建axios实例，在这里可以设置请求的默认配置
  ...
  baseURL: process.env.NODE_ENV === 'production' ? '' : 'http://127.0.0.1:8888/api/private/v1/'   //根据自己配置的反向代理去设置不同环境的baeUrl
})
...
instance.interceptors.response.use(response => {
  ...
  if (response.data.meta.status === 200) {     // 响应结果里的status: ok是我与后台的约定，大家可以根据实际情况去做对应的判断
